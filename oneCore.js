...
export const getLanguage = () => {
    ...
    let navigatorLanguage = (window.navigator.userLanguage || window.navigator.language || 'en').substring(0, 2);//TODO: Rewrite for React Native, no window object
    return localLanguage ? localLanguage : navigatorLanguage;
}
/** 
...
export const updateLanguage = (event) => {
    if(event && event.target) setLanguage(event.target.value);
}
/** 
...
export const readText = (id, language=getLanguage()) => {
    ...
    if(!ONEJS.appText[id][language]) {console.error('No such language: ' + language); return;}
...
// const state = {events: {default: [], source: {local: 'userId'}, 
//     storage: {indexedDB: 'userId'}}, ...};
// 2. Intialize the app() function with the firestore database
// app({template: template, state: state});
// 3. Use the read and update functions to access the data and store a new value
// respectively
// const template = () => [Text()('User name: ' + read('name')),
...
const localStorageRead = (path) => (stateId) => {
    try {        
        ...
        let data = JSON.parse(jsonValue);//Using JSON.parse and stringify() we can store non-string data.
...
export const add = (stateId) => event => {
    let newValue = (event && event.target) ? (event.target.type === 'checkbox' ? event.target.checked : event.target.value) : event;
...
const setupState = (config) => {
    ...
    Object.entries(config).forEach(([stateId, value]) => { 
        ...
        if(value.storage && value.storage.firestore) {
        ...
        else if(value.storage && value.storage.indexedDB) {
        ...
        else if(value.storage && value.storage.local) {
    ...
    Object.entries(config).forEach(([stateId, value]) => {       
        ...
        else if(value.source && value.source.indexedDB) {
        ...
        else if(value.source && value.source.local) {
            localStorageRead(stateId)(value.source.local);
        }
        //If defined by the user, use any function to set the storage. It will be called on write()
        ...
        else if(value.source && typeof value.source === 'function') {
            ONEJS.currentState[stateId].source = value.source;
...
*/
export const BaseComponent = (name, hasChildren=true, ComponentFunctionOrTag) => {
...
* @returns {ReactElement} - The enhanced React element.
*/
export const Component = (name, ComponentFunction) => {
    if(typeof ComponentFunction() === 'function') return CreateComponentWithStructure(name, ComponentFunction);
    else if(typeof ComponentFunction === 'function') return CreateComponentWithoutStructure(name, ComponentFunction);
    return;
}
...
const EnhancedComponent = (ComponentFunctionOrTag) => ({structure, flavor, style, inlineStyle, onInit, onCreate, onDestroy, onPropertyChange, ...attributes}={}) => {    
    //START CLASS SETUP: Framework agnostic
    let classArray = [];
    ...
    if(style) {//style can of Array type. The priority is from left (least priority) to write (most priority)
        if(ONEJS.os === 'web') classArray.push(css(style));
        else if(ONEJS.os === 'native') attributes['stye'] = style;//Sylesheet.create does not seem to provide any performance boost, only validation in dev. https://stackoverflow.com/questions/38886020/what-is-the-point-of-stylesheet-create
    }
    ...
    if(onInit) {//Similar to the deprecated ComponentWillMount. The limitation is that we cannot access the domNode and make changes to it, we need to wait until onCreate
...
export const app = ({name, template, state, theme, themeSetup, text, firestore}) => {
    ...
    else if(ONEJS.os === 'native') return AppComponent;
}

//=============================================================================
// THEME: This module aims to provide a unified and predictable way to define
...
// by the user, should be a CSS variable. 
// CSS variables are given representative names and clustered under 'flavors' which 
...
*/
/** 
* @description Returns all the css theme variables and corresponding values for a certain flavor.
...
*/
export const themeVariable = (variable, value) => {
    return 'var(--one-' + variable + (value ? ', ' + value + ')' : ')');
}
...
export const updateThemeVariable = variable => value => {
    document.body.setProperty('--one-' + variable, value);
}
...
export const setupTheme = ({theme, themeSetup=oneThemeSetup, themeCollection=oneTheme}={}) => {
    ...
    */
    ...
    else if(typeof theme === 'string') theme = themeCollection[theme]; //Selects a certain theme from the collection

    document.body.classList.add(css(themeSetup)); //Adds the main css page to the document body
    ...
    Object.entries(theme).forEach(([flavorId, flavorValue]) => { //Transform each of the themes in css variables stored in a class. This can now be applied to any component
        ONEJS.emotionCSSClasses['flavor'+flavorId] = css(readFlavor(flavorValue));
        // ONEJS.emotionCSSClasses['flavor'+flavorId] = ONEJS.os === 'web' ? css(readFlavor(flavorValue)) : readFlavor(flavorValue);
        if(flavorId === 'default') {
